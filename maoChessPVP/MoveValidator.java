...
{
	public boolean check(ChessBoard gameBoard,String moveString,boolean blackMove)
	{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						Location old = new Location(8 - Character.getNumericValue(firstLocation[1]), Character.getNumericValue((char) (firstLocation[0] - 49)));

						char[] nextLocation = components[2].toCharArray();
						Location next = new Location(8 - Character.getNumericValue(nextLocation[1]), Character.getNumericValue((char) (nextLocation[0] - 49)));

						for (int i = 0; i < 32; i++)
						{
							if (gameBoard.ChessList[i].index.compairTo(old))
							{
								if (gameBoard.ChessList[i].blackSide != blackMove)
								{
									...
									{
									if (gameBoard.ChessList[i].type.contains("ROOK")) {
									...
									if (gameBoard.ChessList[i].type.contains("KNIGHT"))
									{
										if (!gameBoard.checkLocation(next).type.contains("EMPTY") && gameBoard.checkLocation(next).blackSide == blackMove) {
											System.err.println("Can not move to your own chess!");
										}
									...
									if (gameBoard.ChessList[i].type.contains("BISHOP")) {
										if (!gameBoard.checkLocation(next).type.contains("EMPTY") && gameBoard.checkLocation(next).blackSide == blackMove) {
											System.err.println("Can not move to your own chess!");
											return false;
										}
									...
									if (gameBoard.ChessList[i].type.contains("QUEEN")) {
										if (!gameBoard.checkLocation(next).type.contains("EMPTY") && gameBoard.checkLocation(next).blackSide == blackMove) {
											System.err.println("Can not move to your own chess!");
											return false;
										}
										if (old.x == next.x) {
											for (int j = old.y + 1; j < next.y; j++) {
												//if blocked
												Location temp = new Location(old.x, i);
												if (!"EMPTY".equals(gameBoard.checkLocation(temp).type)) {
													System.err.println("Path blocked!");
													return false;
												}
										...
										if (old.y == next.y) {
											for (int j = old.x + 1; j < next.x; j++) {
												//if blocked
												Location temp = new Location(i, old.y);
												if (!"EMPTY".equals(gameBoard.checkLocation(temp).type)) {
													System.err.println("Path blocked!");
													return false;
												}
									...
									if (gameBoard.ChessList[i].type.contains("KING"))
									{
										if (!gameBoard.checkLocation(next).type.contains("EMPTY") && gameBoard.checkLocation(next).blackSide == blackMove)
										{
											System.err.println("Can not move to your own chess!");
                                        ...
                                        if(!gameBoard.checkLocation(temp1).idMoved || !gameBoard.checkLocation(temp2).idMoved)
                                        {
                                            if(next.x==old.x)
                                            {
                                                if(next.y>old.y)
									...
									if(gameBoard.ChessList[i].type.contains("PAWN"))
									{
										if(!gameBoard.checkLocation(next).type.contains("EMPTY") && gameBoard.checkLocation(next).blackSide == blackMove)
										{
											System.err.println("Can not move to your own chess!");
										...
										if(!gameBoard.ChessList[i].idMoved)
										{
											if(next.y==old.y&&abs(next.x-old.x)<=2)
											{
												...
												{
													gameBoard.ChessList[i].enPassant=true;
                                                    gameBoard.move(old,next,1);
													return true;
												}
											}
		...
		System.err.println("Unknown EROR");
		return false;
	}
}
